...
Voronoi::Voronoi(vector<ivec2> &points,
...
{
    int sqrt_nsites = std::sqrt(nsites+4);
...
Halfedge * Voronoi::ELleftbnd(vec2 *p) {
    int i, bucket;
    Halfedge *he;
...
VSite * Voronoi::intersect(Halfedge *el1, Halfedge *el2, vec2 *p) {
    VEdge *e1,*e2, *e;
    Halfedge *el;
    float d, xint, yint;
    int right_of_site;
...
int Voronoi::right_of(Halfedge *el, vec2 *p) {
    VEdge *e;
    VSite *topsite;
...
void Voronoi::clip_line(VEdge *e) {
    ...
    if (!(x1 == x2 && y1==y2)) {
        e->reg[0]->edges.push_back(ivec4(x1, y1, x2, y2));
        e->reg[1]->edges.push_back(ivec4(x1, y1, x2, y2));
    }
}
...
bool voronoi::scomp(VSite lhs, VSite rhs) {
